...
export default function TemplatesObrigacoesPage() {
  ...
  async function criar(e: React.FormEvent) {
    ...
    const res = await fetch("/api/obrigacoes/templates", {
      ...
      body: JSON.stringify({ setorId: novoSetorId, nome: novoNome.trim() }),
    });
    setCriando(false);
    if (res.ok) {
      setNovoNome("");
    ...
    <div className="space-y-5 max-w-2xl">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">
          ...
          use &quot;Gerenciar clientes&quot; dentro da tela do Setor.
        </p>
      ...
      {mostrarForm && (
        <form onSubmit={criar} className="card flex gap-2 items-end flex-wrap">
          <div>
            ...
            <div className="divide-y divide-gray-100">
              {itens.map((t) => (
                <div key={t.id} className="flex items-center gap-3 px-4 py-3">
                  {editandoId === t.id ? (
                    <input className="input flex-1 text-sm" value={nomeEdicao} onChange={(e) => setNomeEdicao(e.target.value)} autoFocus />
                  ) : (
                    <span className={`flex-1 text-sm ${t.ativo ? "text-gray-800" : "text-gray-400 line-through"}`}>{t.nome}</span>
                  )}
                  ...
                  {editandoId === t.id ? (
                    ...
                    <>
                      <button onClick={() => { setEditandoId(t.id); setNomeEdicao(t.nome); }} className="text-xs text-gray-500 hover:underline">Editar</button>
